...
public class KoscomApi {
	...
	public static int currentPrice(String code) throws IOException {
        ...
        urlBuilder.append("?");
        urlBuilder.append(URLEncoder.encode("isuCd","UTF-8") + "=" + URLEncoder.encode(code + ",005930", "UTF-8") + "&");
        urlBuilder.append(URLEncoder.encode("apikey","UTF-8") + "=" + URLEncoder.encode("l7xxc460c2c4fca442ebaef0e2d8977dc043", "UTF-8"));
        URL url = new URL(urlBuilder.toString());
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("GET");
...
//        int index = sb.toString().indexOf(code);
	    int start = sb.toString().indexOf("trdPrc");
	    int end = sb.toString().indexOf("cmpprevddTpCd");
	    int name = Integer.parseInt(sb.toString().substring(start+8, end-2));
